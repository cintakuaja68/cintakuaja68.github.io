...
class Moovie {
  constructor(options) {
    const defaults = {
      ...
      config: {
        ...
        i18n: {
          ...
          main_topic: "settings:",
          main_caption: "Quality",
          main_offset: "Caption Offset",
          main_speed: "Speed",
          main_disabled: "720p",
          main_default: "Default",
          caption_topic: "Quality:",
          caption_back: "Back",
          caption_load: "Quality",
    ...
    options.config == undefined
      ...
      : options.config;
    options.config.controls == undefined
      ? (options.config.controls = defaults.config.controls)
      : options.config.controls;
    for (var key in defaults.config.controls) {
      options.config.controls[key] == undefined
        ? (options.config.controls[key] = defaults.config.controls[key])
        : (options.config.controls[key] = options.config.controls[key]);
    }

    // Get default storage
    options.config.storage == undefined
      ? (options.config.storage = defaults.config.storage)
      : options.config.storage;
    for (var key in defaults.config.storage) {
      options.config.storage[key] == undefined
        ? (options.config.storage[key] = defaults.config.storage[key])
        : (options.config.storage[key] = options.config.storage[key]);
    }

    // Get default i18n
    options.config.i18n == undefined
      ? (options.config.i18n = defaults.config.i18n)
      : options.config.i18n;
    for (var key in defaults.config.i18n) {
      options.config.i18n[key] == undefined
        ? (options.config.i18n[key] = defaults.config.i18n[key])
        : (options.config.i18n[key] = options.config.i18n[key]);
    }

    this.selector =
      options.selector.substring(1) || defaults.selector.substring(1);
    ...
    var mainmenu = (this.mainmenu = [
      {
        name: "play_button",
        element:
          "<button class='player__button toggle' id='tooglebutton_" +
          randomID +
          "'><img id='moovie_bplay_" +
          randomID +
          "' src='" +
          icons.path +
          "play.svg'></button>",
      ...
      },
      {
        name: "progress_bar",
        element:
          "<div class='moovie_cuetime' id='moovie_cue_timer_" +
          randomID +
          "'>loading...</div><div id='moovie_moovie_el_progressbar_" +
          randomID +
          "' class='moovie_progress player__slider' top:15px;><input type='range' id='range_progress_" +
          randomID +
          "' class='styled-slider slider-progress' min='0' value='0' step='0.01' autocomplete='off' style='width: 100%; cursor:pointer;' /><canvas id='moovie_buffered_" +
          randomID +
          "' style='position: absolute; bottom: 14px; left: 0; opacity: 0.4;  width: 100%; height:5px;'></canvas></div>",
        opcional: false,
        tooltip: false,
      },
      {
        name: "playtime",
        element:
          "<div id='moovie_el_current_" +
          randomID +
          "' class='player__button player_button_disabled moovie_currentime'><span id='moovie_currentime_" +
          randomID +
          "'>00:00</span> / <span ' id='moovie_fulltime_" +
          randomID +
          "'></span></div>",
        opcional: true,
        tooltip: false,
      },
      {
        name: "mute",
        element:
          "<button id='mooviegrid_mute_" +
          randomID +
          "' class='player__button'><img id='icon_volume_" +
          randomID +
          "' src='" +
          icons.path +
          "volume.svg'></button>",
      ...
      },
      {
        name: "volume",
        element:
          "<input type='range' id='mooviegrid_volume_" +
          randomID +
          "' style='max-width:100px; min-width:50px;' name='volume' class='moovie_progress_sound' min=0 max='1' step='0.01' value='1'>",
        opcional: true,
        tooltip: false,
      },
      {
        name: "subtitles",
        element:
          "<button id='moovie_subtitle_" +
          randomID +
          "' style='margin-left:5px' class='player__button'><img class='opacity_svg' id='moovie_subtitle_svg_" +
          randomID +
          "' src='" +
          icons.path +
          "cc.svg'></button>",
      ...
      },
      {
        name: "config",
        element:
          "<button id='moovie_el_cog_" +
          randomID +
          "' class='player__button'><img src='" +
          icons.path +
          "cog.svg'></button>",
        opcional: true,
      ...
      },
      {
        name: "fullscreen",
        element:
          "<button id='fullscreen_" +
          randomID +
          "' class='player__button fullscreen_button'><img src='" +
          icons.path +
          "fullscreen.svg'></button>",
        opcional: true,
      ...
      },
      {
        name: "Main Submenu",
        element:
          "<div style='display:none;' class='moovie_submenu menuclosed' id='moovie_submenu_" +
          randomID +
          "'></div>",
        opcional: false,
        tooltip: false,
    ...
    var submenu = (this.submenu = {
        ...
        elements: [
          {
            name: "topic",
            element:
              "<li class='topic_submenu'>" + this.i18n.main_topic + "</li>",
            opcional: false,
          },
          {
            name: "submenuCaptions",
            element:
              "<li id='this.moovie_el_topicCaption_" +
              randomID +
              "'><span>" +
              this.i18n.main_caption +
              "</span><span class='option_submenu' id='option_submenu_caption_" +
              randomID +
              "'>" +
              this.i18n.main_disabled +
              "</span></li>",
            opcional: true,
          },
          {
            name: "submenuOffset",
            element:
              "<li id='moovie_el_capoffset_" +
              randomID +
              "'><span >" +
              this.i18n.main_offset +
              "</span><span class='option_submenu' id='option_submenu_range_" +
              randomID +
              "'>0s</span></li>",
            opcional: true,
          },
          {
            name: "submenuSpeed",
            element:
              "<li id='topic_submenu_speed_" +
              randomID +
              "'><span>" +
              this.i18n.main_speed +
              "</span><span class='option_submenu' id='option_submenu_speed_" +
              randomID +
              "'>" +
              this.i18n.main_default +
              "</span></li>",
            opcional: true,
          },
        ],
      ...
      captionSubmenu: {
        mainElement:
          "<ul style='display:none;' id='moovie_submenu_captions_" +
          randomID +
          "'><input style='display:none;' type='file' id='localsub_" +
          randomID +
          "'><li class='topic_submenu'>" +
          this.i18n.caption_topic +
          "</li><li id='moovie_el_capback_" +
          randomID +
          "' style='font-weight:bold;'>" +
          this.i18n.caption_back +
          "</li></ul>",
      ...
      rangeSubmenu: {
        mainElement:
          "<ul style='display:none; width:250px;' id='moovie_range_captions_" +
          randomID +
          "'><li class='topic_submenu'>" +
          this.i18n.offset_topic +
          "<output style='position:absolute; right:22px;' id='valoffset_" +
          randomID +
          "'>0</output></li><li class='topic_submenu'><span>-5s</span><span style='float: right;'>+5s</span><input type='range' oninput='valoffset_" +
          randomID +
          ".value = offset_range_input_" +
          randomID +
          ".value' id='offset_range_input_" +
          randomID +
          "' min='-5' max='5' step='0.2'></li>",
        parentID: "moovie_range_captions_",
      },
      speedSubmenu: {
        mainElement:
          "<ul style='display:none; width:250px;' id='moovie_range_speed_" +
          randomID +
          "'><li class='topic_submenu'>" +
          this.i18n.speed_topic +
          "<output style='position:absolute; right:22px;' id='valoffset_speed_" +
          randomID +
          "'>1</output></li><li class='topic_submenu'><span>0.1x</span><span style='float: right;'>8x</span><input type='range' value='1' oninput='valoffset_speed_" +
          randomID +
          ".value = offset_range_speed_" +
          randomID +
          ".value' id='offset_range_speed_" +
          randomID +
          "' min='0.1' max='8' step='0.1'></li>",
     ...
     */
    var handleCustomEvents = (this.handleCustomEvents =
      function handleCustomEvents() {
        ...
        for (var i = 0; i < customEvents.length; i++) {
          ...
          if (
            customEvents[i].hasOwnProperty("fired") &&
            customEvents[i].fired
          ) {
            // Remove event after its time
            if (video.currentTime > customEvents[i].endtime) {
              customEvents[i].fired = false;
              document
                .getElementById("moovie_ce_" + customEvents[i].eventID)
                .remove();
            }
          } else {
            if (
              video.currentTime > customEvents[i].starttime &&
              video.currentTime < customEvents[i].endtime
            ) {
              // Assign new keys and infos
              Object.assign(customEvents[i], { fired: true });
              Object.assign(customEvents[i], { eventID: eventID });

              moovie_el_video.insertAdjacentHTML(
                "beforeend",
                "<div id='moovie_ce_" +
                  eventID +
                  "' class='moovie_cevent " +
                  customEvents[i].position +
                  " " +
                  customEvents[i].class +
                  "'>" +
                  customEvents[i].content +
                  "</div>"
              );
              let eventElement = document.getElementById(
                "moovie_ce_" + eventID
              );
              ...
              switch (customEvents[i].type) {
                case "skip":
                  if (
                    customEvents[i].to <= video.duration &&
                    !isNaN(customEvents[i].to) &&
                    customEvents[i].to
                  ) {
     ...
     */
    var handleStorage = (this.handleStorage = function handleStorage(
      order,
      variable,
      value
    ) {
      switch (order) {
        ...
        case "setStorage":
          ...
          if (config["storage"]["captionSize"]) {
            document.getElementById(
              "caption_track_" + randomID
            ).style.fontSize = localStorage.getItem("captionSize") + "px";
          }
          break;
      }
    });
    ...
    var togglePlay = (this.togglePlay = function togglePlay() {
      ...
      document.getElementById(
        "poster_layer_" + randomID
      ).style.backgroundImage = "none";
      if (video.paused == true) {
        video.play();
        document.getElementById("moovie_bplay_" + randomID).src =
          icons.path + "pause.svg";
        togglePoster("hide");
    ...
    var SetFullScreen = (this.SetFullScreen = function SetFullScreen(order) {
      switch (order) {
        case "toggleFullscreen":
          ...
          } else if (moovie_el_player.webkitRequestFullscreen) {
            // Safari
            moovie_el_player.webkitRequestFullscreen();
          } else if (moovie_el_player.msRequestFullscreen) {
            // IE11
    ...
    function bufferedVideo() {
      ...
      var b = _this.video.buffered, // Buffer object
        ...
        x1,
        x2; // buffer segment mark positions
    ...
    function updateTime() {
      ...
      if (moovie_ishiden == 0) {
        document.getElementById("moovie_currentime_" + randomID).innerHTML =
          player_time(video.currentTime);
      }
    ...
    function localSubtitles() {
      ...
      let promise = new Promise(async function (resolve, reject) {
        ...
        if (allowedFormats.includes(CheckFormat)) {
          document
            .querySelectorAll(
              "#moovie_submenu_captions_" + randomID + " .caption_track"
            )
            .forEach((item) => {
              item.remove();
            });
          ...
          console.log("Only .vtt and .srt formats are allowed.");
        }
      });
    ...
    var OffsetChange = (this.OffsetChange = function OffsetChange() {
      offsettime = document.getElementById(
        "offset_range_input_" + randomID
      ).value;
      document.getElementById("option_submenu_range_" + randomID).innerHTML =
        offsettime + "s";
      document.getElementById("valoffset_" + randomID).value = offsettime;

      // Save info in internal storage if true
      if (config["storage"]["captionOffset"]) {
        handleStorage("set", "captionOffset", offsettime);
      }
    ...
    var SpeedChange = (this.SpeedChange = function SpeedChange() {
      ...
      if (speed == 1) {
        document.getElementById("option_submenu_speed_" + randomID).innerHTML =
          _this.i18n.main_default;
      } else {
        document.getElementById("option_submenu_speed_" + randomID).innerHTML =
          speed + "x";
      }
      ...
      // Save info in internal storage if true
      if (config["storage"]["playrateSpeed"]) {
        handleStorage("set", "playrateSpeed", speed);
      }
    });
    ...
    function Scrub(e) {
      if (!detectTouchScreen()) {
        ...
        let scrubTime =
          (offsetEvent / moovie_elprogress.offsetWidth) * video.duration;
        if (scrubTime >= video.duration) {
          scrubTime = video.duration;
        }
        video.currentTime = scrubTime;
      } else {
        var offsetEvent = document.getElementById(
          "range_progress_" + randomID
        ).value;
     ...
     ** Change Tooltip
     */
    var ChangeTooltip = (_this.ChangeTooltip = function ChangeTooltip(
      elemName,
      defaultState
    ) {
      // Get tooltip element
      ...
      if (toolText.indexOf("(") > -1) {
        ...
        var betweenBrackets = toolText.substring(
          toolText.lastIndexOf("(") + 1,
          toolText.lastIndexOf(")")
        );
      ...
      if (finalTooltip) ele.innerText = finalTooltip;
      ...
      if (
        ...
        _this.moovie_el_controlbar.offsetWidth
      ) {
     ...
     ** Activate Subtitles
     */
    var ActivateSubtitles = (this.ActivateSubtitles =
      function ActivateSubtitles() {
        if (hassubtitles == 1) {
          if (subtitles == 0) {
            subtitles = 1;
            document
              .getElementById("moovie_subtitle_svg_" + randomID)
              .classList.remove("opacity_svg");
     ...
     */
    var checkSoundLevel = (this.checkSoundLevel = function checkSoundLevel(
      order
    ) {
      switch (order) {
        case "checkMute":
          if (video.muted || video.volume == 0) {
            ...
            document.getElementById("icon_volume_" + randomID).src =
              icons.path + "mute.svg";
            ChangeTooltip("mute", 1);
          } else {
            document.getElementById("icon_volume_" + randomID).src =
              icons.path + "volume.svg";
        ...
        case "toogleMute":
          if (video.muted == true) {
            checkSoundLevel("unmutePlayer");
          } else {
            checkSoundLevel("mutePlayer");
          }
          break;
        case "checkStorage":
    ...
    var Submenu = (this.Submenu = function Submenu(order) {
      switch (order) {
        ...
        case "ORange": // Close caption menu
          moovie_el_submain.style.display = "none";
          document.getElementById(
            "moovie_range_captions_" + randomID
          ).style.display = "block";
     ...
     */
    var TransformPlayer = (this.TransformPlayer = function TransformPlayer(
      caption
    ) {
      ...
      if (containerWidth <= 460) {
        ...
        document.getElementById("moovie_el_current_" + randomID).style.display =
          "none";
        document
          .getElementById("moovie_moovie_el_progressbar_" + randomID)
          .classList.add("responsive_bar");
        ...
        document
          .getElementById("moovie_moovie_el_progressbar_" + randomID)
          .classList.remove("responsive_bar");
        document.getElementById("moovie_fulltime_" + randomID).innerHTML =
          player_time(video.duration);
    ...
    var SetCaptionSize = (this.SetCaptionSize = function SetCaptionSize(value) {
      ...
      if (!value) {
        ...
        var fontSizeCap = containerSize * 0.1;
        ...
        // Save info in internal storage if true
        if (config["storage"]["captionSize"]) {
          handleStorage("set", "captionSize", computedFontSize);
        }
      }
    });
    ...
    var PlayCaption = (this.PlayCaption = function PlayCaption(caption) {
      ...
      function RunCaption() {
        for (var i = 1; i < selectedCaption.length; i++) {
          if (
            selectedCaption[i]["starttime"] != undefined &&
            selectedCaption[i]["endtime"] != undefined
          ) {
            ...
            if (offsettime >= 0) {
              ...
              var RealcurrentTime =
                video.currentTime + Number(offsettimeBelow0);
            }
            ...
            if (
              RealcurrentTime >= cap_starttime &&
              RealcurrentTime <= cap_endtime &&
              subtitles == 1
            ) {
              ...
              if (selectedCaption[i]["text2"] != undefined) {
                document.getElementById("caption_track_" + randomID).innerHTML =
                  selectedCaption[i]["text1"] +
                  "<br>" +
                  selectedCaption[i]["text2"];
              } else {
                document.getElementById("caption_track_" + randomID).innerHTML =
                  selectedCaption[i]["text1"];
              }
            ...
            if (RealcurrentTime > cap_endtime) {
              document.getElementById("caption_track_" + randomID).innerHTML =
                "";
            }
          }
      ...
      if (caption["attributes"]["format"]) {
        var capFormat =
          caption["attributes"]["src"]["nodeValue"] +
          caption["attributes"]["format"]["nodeValue"];
      } else {
      ...
      if (hassubtitles == 1) {
        video.removeEventListener("timeupdate", RunCaption, true);
      } else {
    ...
    var SetCaptions = (this.SetCaptions = function SetCaptions(caption) {
      // Get Caption Format
      let CaptionFormat = caption["attributes"]["src"]["nodeValue"].substr(
        caption["attributes"]["src"]["nodeValue"].length - 4
      );
      ...
      if (caption["attributes"]["format"]) {
        CaptionFormat = caption["attributes"]["format"]["nodeValue"];
      }

      // Add caption to list and add event listener to play it
      moovie_el_captions.insertAdjacentHTML(
        "beforeend",
        "<li class='caption_track' id='captionid_" +
          captionIDRandom +
          "'><span>" +
          capLabel +
          "</span><span class='labelformat'>" +
          CaptionFormat +
          "</span></li>"
      );
      document
        .getElementById("captionid_" + captionIDRandom)
        .addEventListener("click", function () {
          PlayCaption(caption);
          document.getElementById(
            "option_submenu_caption_" + randomID
          ).innerHTML = capLabel;
    ...
    var GetCaptions = (this.GetCaptions = function GetCaptions() {
      ...
      var vcaptions = document
        .getElementById(_this.selector)
    ...
    var Keybinds = (this.Keybinds = function Keybinds() {
      ...
      video.addEventListener("keydown", function (event) {
        ...
        if (
          event.keyCode == 32 ||
          (event.shiftKey && event.keyCode == 87) ||
          (event.shiftKey && event.keyCode == 83)
        ) {
          event.preventDefault();
        }
        if (event.shiftKey) {
          ShortCut = acceptedShortcuts["keycode_shift" + event.keyCode];
        } else {
          ShortCut = acceptedShortcuts["keycode_" + event.keyCode];
        }
    ...
    var SetupLogic = (this.SetupLogic = function SetupLogic() {
      ...
      // volume element
      moovie_ul_soundv = document.getElementById(
        "mooviegrid_volume_" + randomID
      );

      // Mute
      document.getElementById("mooviegrid_mute_" + randomID).addEventListener(
        "click",
        function () {
          checkSoundLevel("toogleMute");
        },
      ...
      // Submenu events
      this.control_buttons.config.addEventListener(
        "click",
        function () {
      ...
      this.moovie_el_topicCaption.addEventListener(
        "click",
        function () {
          Submenu("OCaption");
        },
      ...
      this.moovie_el_topicSpeed.addEventListener(
        "click",
        function () {
          Submenu("OSpeed");
        },
      ...
      this.moovie_el_capback.addEventListener(
        "click",
        function () {
          Submenu("CCaption");
        },
      ...
      this.control_buttons.subtitles.addEventListener(
        "click",
        ActivateSubtitles
      );
      moovie_el_video.addEventListener(
        "mouseleave",
        function () {
          Submenu("CAll");
        },
      ...
      this.moovie_el_capoffset.addEventListener(
        "click",
        function () {
          Submenu("ORange");
        },
      ...
      if (!detectTouchScreen()) {
        ...
        moovie_elprogress.addEventListener(
          "mousemove",
          (e) => mousedown && Scrub(e)
        );
        ...
        // Poster Functions
        moovie_el_poster = moovie_el_player.querySelector(
          "#poster_layer_" + randomID
        );
        ...
        moovie_el_locally.addEventListener(
          "click",
          function () {
        ...
        // Cue Time
        document
          .getElementById("range_progress_" + randomID)
        ...
        moovie_el_controls.addEventListener("mouseover", (e) => {
          isopen = 1;
        });
        ...
        moovie_el_progress.addEventListener(
          "input",
          function (event) {
            video.pause();
          },
        ...
        moovie_el_poster.addEventListener("touchend", function (event) {
          if (!video.pause()) {
            togglePlay();
          }
        });
    ...
    var SetupMenu = (this.SetupMenu = function SetupMenu() {
      ...
      for (var i = 0; i < mainmenu.length; i++) {
        if (
          (this.config.controls[mainmenu[i].name] != undefined &&
            this.config.controls[mainmenu[i].name] == true) ||
          mainmenu[i].opcional == false
        ) {
          moovie_el_controls.insertAdjacentHTML(
            "beforeend",
            mainmenu[i].element
          );
        } else {
          moovie_el_controls.insertAdjacentHTML(
            "beforeend",
            "<div style='display:none;'>" + mainmenu[i].element + "</div>"
          );
        }
      }
      ...
      for (var i = 0; i < mainmenu.length; i++) {
        if (mainmenu[i].tooltip) {
          ...
          document
            .getElementById(doc["body"]["firstChild"]["id"])
            .insertAdjacentHTML(
              "beforeend",
              "<div id='moovie_" +
                mainmenu[i].name +
          ...
          let targetNodeTool = document.getElementById(
            "moovie_" + mainmenu[i].name + "_tooltip_" + randomID
          );
      ...
      for (let key in submenu) {
        moovie_el_submenu.insertAdjacentHTML(
          "beforeend",
          submenu[key]["mainElement"]
        );

        if (submenu[key].hasOwnProperty("elements")) {
          for (var i = 0; i < submenu[key]["elements"].length; i++) {
            if (
              (this.config.controls[submenu[key]["elements"][i]["name"]] !=
                undefined &&
                this.config.controls[submenu[key]["elements"][i]["name"]] ==
                  true) ||
              submenu[key]["elements"][i]["opcional"] == false
            ) {
              document
                .getElementById(submenu[key]["parentID"] + randomID)
                .insertAdjacentHTML(
                  "beforeend",
                  submenu[key]["elements"][i]["element"]
                );
            } else {
              document
                .getElementById(submenu[key]["parentID"] + randomID)
      ...
      moovie_el_progress = document.getElementById(
        "range_progress_" + randomID
      );
      moovie_el_captions = document.getElementById(
        "moovie_submenu_captions_" + randomID
      );
      moovie_el_range = document.getElementById(
        "moovie_range_captions_" + randomID
      );
      moovie_el_speed = document.getElementById(
        "moovie_range_speed_" + randomID
      );

      // Update variables
      this.moovie_el_topicCaption = document.getElementById(
        "this.moovie_el_topicCaption_" + randomID
      );
      this.moovie_el_topicSpeed = document.getElementById(
        "topic_submenu_speed_" + randomID
      );
      this.moovie_el_capback = document.getElementById(
        "moovie_el_capback_" + randomID
      );
      this.moovie_el_capoffset = document.getElementById(
        "moovie_el_capoffset_" + randomID
      );
      this.moovie_el_speedinput = moovie_el_sinput = document.getElementById(
        "offset_range_speed_" + randomID
      );
      this.moovie_el_rangeinput = moovie_el_rinput = document.getElementById(
        "offset_range_input_" + randomID
      );
      this.moovie_el_progressbar = moovie_el_progress;
      this.moovie_el_volume = moovie_el_volume = document.getElementById(
        "mooviegrid_volume_" + randomID
      );
      this.moovie_el_cuetimer = moovie_el_cuetimer = document.getElementById(
        "moovie_cue_timer_" + randomID
      );
      this.moovie_el_submenuBase = moovie_el_submenu;
      this.moovie_el_localsub = moovie_el_localsub = document.getElementById(
        "localsub_" + randomID
      );
      this.moovie_el_locally = moovie_el_locally = document.getElementById(
        "locally_" + randomID
      );
      this.moovie_el_submain = moovie_el_submain = document.getElementById(
        "moovie_submenu_main_" + randomID
      );
      this.moovie_el_buffered = moovie_el_buffered = document.getElementById(
        "moovie_buffered_" + randomID
      );

      // Control buttons
      this.control_buttons = {
        play:
          (this.moovie_el_play = moovie_el_play =
            document.getElementById("tooglebutton_" + randomID)),
        mute:
          (this.moovie_el_mute = moovie_el_mute =
            document.getElementById("mooviegrid_mute_" + randomID)),
        subtitles:
          (this.moovie_el_subtitles = moovie_el_subtitles =
            document.getElementById("moovie_subtitle_" + randomID)),
        config: (this.moovie_el_cog = document.getElementById(
          "moovie_el_cog_" + randomID
        )),
        fullscreen:
          (this.moovie_el_fullscreen = moovie_el_fullscreen =
            document.getElementById("fullscreen_" + randomID)),
      };
    });
    ...
    var SetupPlayer = (this.SetupPlayer = function SetupPlayer() {
      ...
      if (
        vsource == null &&
        this.element.getElementsByTagName("source")[0] != undefined
      ) {
        vsource = this.element.getElementsByTagName("source")[0].src;
      } else {
        console.log(
          "No video source found. Read documentation to a add video source dynamically."
        );
      }

      // Get poster if exists
      var vposter = document
      ...
      this.moovie_el_video = moovie_el_video = document.getElementById(
        "moovie__video_" + randomID
      );
      ...
      moovie_el_video.insertAdjacentHTML(
        ...
        "<video tabindex='1' id='moovie_vid_" +
          randomID +
          "' preload='auto' class='player__video viewer' style='width:100%; height:100%;' src='" +
          vsource +
          "' playsinline></video>"
      );
      ...
      moovie_el_video.insertAdjacentHTML(
        "beforeend",
        "<div style='opacity:0;' id='moovie__controls_" +
          randomID +
          "' class='moovie_controls'></div>"
      );
      this.moovie_el_controlbar = moovie_el_controls = document.getElementById(
        "moovie__controls_" + randomID
      );
      ...
      moovie_el_video.insertAdjacentHTML(
        "afterbegin",
        "<div id='medialoading_" +
          randomID +
          "' class='loadingv'><div class='loading animated fadeIn'><div class='moovie_bg'></div></div></div><div class='poster_layer posteron' id='poster_layer_" +
          randomID +
          "'></div>"
      );
      this.medialoading = document.getElementById("medialoading_" + randomID);

      this.moovie_el_poster = moovie_el_poster = document.getElementById(
        "poster_layer_" + randomID
      );
      ...
      moovie_el_video.insertAdjacentHTML(
        "beforeend",
        "<div class='moovie_captionspot caption_size'><p class='moovie_caption' id='caption_track_" +
          randomID +
          "'></p></div>"
      );
  ...
  addTrack(properties) {
    ...
    if (properties.options && typeof properties.options === "object") {
      ...
      for (var i = 0; i < prop; i++) {
        ...
        if (properties.options[i].label) {
          addlabel = "label='" + properties.options[i].label + "'";
        } else {
          addlabel = "label='New Subtitle'";
        }
        if (properties.options[i].srclang) {
          srclang = "srclang='" + properties.options[i].srclang + "'";
        } else {
          srclang = "srclang='New'";
        }
        if (properties.options[i].src) {
          tracksrc = "src='" + properties.options[i].src + "'";
        } else {
          tracksrc = "";
        }

        if (!tracksrc) {
          console.log("Error, 'src' can not be empty.");
        } else {
          document
            .getElementById(this.selector)
            .insertAdjacentHTML(
              "beforeend",
              "<track " + addlabel + " " + srclang + " " + tracksrc + ">"
            );
        }
      }
      ...
      document
        .querySelectorAll(
          "#moovie_submenu_captions_" + this.randomID + " .caption_track"
        )
        .forEach((item) => {
          item.remove();
        });
      ...
      if (typeof properties.onComplete == "function") {
  ...
  change(properties) {
    ...
    if (properties.video && typeof properties.video === "object") {
      ...
      if (properties.video.posterSrc) {
        ...
        this.moovie_el_poster.style.backgroundImage =
          "url(" + properties.video.posterSrc + ")";
      }
  ...
  // Get stopped state
  get stopped() {
    return Boolean(this.video.paused && this.video.currentTime === 0);
  }
  // Get ended state
  get ended() {
  ...
  get duration() {
    ...
    const duration =
      realDuration || realDuration === Infinity ? 0 : realDuration;
  ...
  // Set speed
  set speed(input) {
    if (input < -0.1 || input > 8) {
      return "Value must be between -0.1 and 8";
    } else {
  ...
  set captionOffset(input) {
    if (input < -5 || input > 5) {
      return "Value must be between -5 and 5";
    } else {
  ...
  set muted(input) {
    if (input) {
      this.checkSoundLevel("mutePlayer");
    } else {
      this.checkSoundLevel("unmutePlayer");
    }
  }
}
